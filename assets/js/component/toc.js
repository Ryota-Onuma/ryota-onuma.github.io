...
const updateActiveClasses = () => {
  const headers = document.querySelectorAll(
    "#article h1:not(.article-title), #article h2:not(.article-title), #article h3:not(.article-title)",
  );
  const links = document.querySelectorAll("#TableOfContents a");
  for (let i = 0; i < headers.length; i++) {
    const rect = headers[i].getBoundingClientRect();
    const nextRect = headers[i + 1]
      ? headers[i + 1].getBoundingClientRect()
      : null;
    if (
      (rect.top >= 0 && rect.top <= window.innerHeight) ||
      (rect.top < 0 && nextRect && nextRect.top > window.innerHeight)
    ) {
